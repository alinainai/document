...
## 4、RequestBuilder#into(ImageView) 流程

### 4.1 RequestBuilder#into(ImageView) 方法
...
```
### 4.2 glideContext.buildImageViewTarget(view, transcodeClass) 方法
...
private <Y extends Target<TranscodeType>> Y into(@NonNull Y target, @Nullable RequestListener<TranscodeType> targetListener, BaseRequestOptions<?> options, Executor callbackExecutor) {
    ...
    Request request = buildRequest(target, targetListener, options, callbackExecutor); // 生成一个 Request 对象
    Request previous = target.getRequest();
    if (request.isEquivalentTo(previous) && !isSkipMemoryCacheWithCompletePreviousRequest(options, previous)) { // 先判断是否存在 request
        if (!Preconditions.checkNotNull(previous).isRunning()) { // 存在的 request 没有运行
            previous.begin();
        }
        return target;
    }
