...
`load(string)` 方法内部通过 asDrawable() 返回一个 `RequestBuilder<Drawable>` 对象。然后调用 RequestBuilder 的 load 方法。

我们继续看 `RequestBuilder#load`方法
```java
...
private RequestBuilder<TranscodeType> loadGeneric(@Nullable Object model) {
    ...
    this.model = model;
    isModelSet = true;
    return selfOrThrowIfLocked(); // 给 model 赋值后返回 this 本身
}
```
RequestManager.load(string) 返回一个 RequestBuilder 对象，RequestBuilder 类是处理设置项和通用资源开始加载的类。`<TranscodeType>` 泛型表示传递给 Target 的资源类型。
...
```
好多配置项已经在 `GlideBuilder` 中初始化了，`Glide` 构造的核心代码是创建一个 `registry`，然后调用它的 append，register 函数设置一些参数。这些参数，主要作用就是对后面的图片进行 `loading、decoding、encoding`。
