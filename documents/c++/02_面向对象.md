...
public://声明运算符重载
    Artical operator+(const Artical &A) const;
    void display() const;
private:
    string m_content;
};
...
public:
    ...
    Artical(string content);
public:
    //运算符重载函数不是 Artical 类的成员函数，但是却用到了 Artical 类的 private 成员变量，所以必须在 Artical 类中将该函数声明为友元函数。
    friend Artical operator+(const Artical &A, const Artical &B);
    void display() const;
private:
    string m_content;
};

Artical operator+(const Artical &A, const Artical &B);
...
Artical operator+(const Artical &A, const Artical &B){
    Artical C;
    C.m_content = A.m_content + B.m_content;
    return C;
}
